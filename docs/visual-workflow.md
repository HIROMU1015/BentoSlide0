...
In standard whole-deck HTML authoring, a visual change uses a temporary full-deck candidate. Before applying it, Work/GPT reports the requested slide and every related slide affected by shared composition, narrative, registry/provenance, assets, or global style; explicit confirmation is required. Position or size changes after conversion can be made in Bento. The section digest still includes referenced visual definitions, transitive figure-to-asset dependencies, source/origin metadata, and asset bytes, so it serves as an internal impact scope. Changing a referenced locator or generated description invalidates that section; global styling or structural changes require complete-deck review. The optional rolling route retains targeted section promotion and acceptance.
