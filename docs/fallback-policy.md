...
- `auto`: try the native mapping; on a mapping failure, use localized SVG.
...
Auto-correction is conservative. Out-of-bounds frames are clamped and minor text overflow can reduce font size; source wording is immutable. Overlap is reported rather than blindly rearranged because intentional overlays, labels, and connector geometry are common. A developer should update the HTML source when a diagnostic reflects an authoring defect.
