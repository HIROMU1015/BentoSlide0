...
For new schema v2 single/imported decks, the default is `authoring.strategy: whole_deck`: create the complete fixed-size HTML/registry deck, show one whole-deck preview, then revise it conversationally. Do not edit reviewed canonical HTML directly. Build a temporary candidate, calculate requested/changed/related slides plus structural, registry, and global-style impact, tell the user exactly what will change and what else must be rechecked, and ask whether that candidate is acceptable. Only an explicit confirmation may approve and transactionally apply the exact revision-bound candidate; the approval digest also binds the human explanation. Reject stale candidates or changed impact evidence, and revalidate canonical/candidate inputs under one union writer lease at apply. After applying, browser-check every affected slide that remains in the deck for layout, overflow, visual consistency, and narrative continuity, persist revision-bound report/environment/screenshot evidence, and do not ask for whole-deck approval until it is current.
